    ...
    def _load_translations(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            "ru": {
                ...
                "flood_wait_error": "Превышен лимит запросов для сообщения[{id}], ожидание {seconds} сек...",
                "file_reference_expired_skip": "Ссылка на файл истекла для сообщения[{id}] после 3 попыток, пропуск загрузки.",
                "file_migrate_error": "Файл находится в DC {dc} для сообщения[{id}], переключение DC, повтор через 10 секунд...",
                "file_migrate_error_skip": "Не удалось переключиться на DC {dc} для сообщения[{id}] после 3 попыток, пропуск загрузки.",
                "connection_error": "Обрыв соединения при загрузке сообщения[{id}]: {error}, повтор через 10 секунд...",
                "connection_error_skip": "Обрыв соединения для сообщения[{id}] после 3 попыток: {error}, пропуск загрузки.",
            ...
            "en": {
                ...
                "flood_wait_error": "Rate limit exceeded for message[{id}], waiting for {seconds} seconds...",
                "file_reference_expired_skip": "File reference expired for message[{id}] after 3 retries, download skipped.",
                "file_migrate_error": "File is in DC {dc} for message[{id}], switching DC, retrying after 10 seconds...",
                "file_migrate_error_skip": "Failed to switch to DC {dc} for message[{id}] after 3 retries, download skipped.",
                "connection_error": "Connection error when downloading message[{id}]: {error}, retrying after 10 seconds...",
                "connection_error_skip": "Connection error for message[{id}] after 3 retries: {error}, download skipped.",
