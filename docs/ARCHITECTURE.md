...
К базовой системе были добавлены следующие продвинутые модули:

### 🌐 Web Dashboard (FastAPI + React)
- **Backend (FastAPI)**: Асинхронный сервер, предоставляющий API и WebSockets для реал-тайм мониторинга.
- **Frontend (React)**: Современный SPA с премиальным дизайном, использующий Recharts для визуализации и Framer Motion для анимаций.
- **WebSocket**: Живой поток данных из `rich.progress` транслируется напрямую в браузер.

### 📊 ClickHouse Integration
- **Аналитическое хранилище**: Все метаданные сообщений и чатов дублируются в ClickHouse.
- **Производительность**: Позволяет мгновенно получать статистику по миллионам сообщений.
- **Визуализация**: Дашборд использует данные из ClickHouse для построения графиков активности и распределения данных. Блок «Chat Statistics» (список чатов с объёмом и числом сообщений) строится агрегацией по таблице `messages` (count, sum(file_size)), а не из таблицы `chats`.
- **primary_source** (опция в `clickhouse.primary_source`): при `true` архив и статистика считаются первичным источником истины: проверка дублей, список чатов, генерация HTML и экспорт (`export_chat`) читают данные из ClickHouse; JSONL при этом может не использоваться для чтения.
- **Проверка доступа при старте**: если задан `primary_source: true`, при запуске выполняется проверка подключения к ClickHouse. При ошибке доступа приложение не завершается — в лог пишется предупреждение, все операции с БД отключаются, работа идёт без ClickHouse (источники — JSONL и локальные файлы).
