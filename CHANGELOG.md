...
- **Веб-дашборд: фильтр чатов по наличию сообщений не работал**
  ...
  - Исправлен порядок применения фильтров на фронтенде: фильтр `showAllChats` теперь корректно показывает все чаты или только с сообщениями
...
- **Исправлена логика сохранения JSONL архива при включенном ClickHouse**
  - При `clickhouse.primary_source: true` JSONL архив не создавался для новых сообщений
  - Причина: проверка дублей в ClickHouse происходила ПОСЛЕ сохранения сообщений в БД
  - Последовательность была: save_message(CH) → flush(CH) → save_batch(JSONL) → проверка дублей в CH
  - Проверка `get_existing_message_ids()` всегда находила только что сохраненные сообщения
  - Результат: "Архив чата уже содержит все сообщения (ClickHouse)" и пропуск записи в JSONL
  - Исправление: удалена проверка дублей по ClickHouse, используется только проверка в самом JSONL файле
  - Теперь JSONL архив корректно создаётся параллельно с ClickHouse при любом режиме
