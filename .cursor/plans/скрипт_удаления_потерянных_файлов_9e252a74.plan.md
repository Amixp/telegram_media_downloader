...
todos:
  ...
  - id: add-config-option
    content: Добавить опцию --config для чтения base_directory и history_directory из config.yaml
    status: completed
  - id: log-missing-archived
    content: Логировать файлы из архива, которых нет на диске (--no-log-missing отключает)
    status: completed
  - id: remove-empty-dirs
    content: Опция --remove-empty-dirs: удалять пустые папки в папках медиа после удаления файлов
    status: completed
isProject: false
---
...
- **Файлы из архива, которых нет на диске:** по умолчанию логируются (warning + при `-v` список); флаг `--no-log-missing` отключает. В статистике: «В архиве, но нет на диске: N».
- Символические ссылки (проверять `os.path.islink()`, не сканировать)
- **Пустые папки после удаления:** опция `--remove-empty-dirs` — удаляет пустые каталоги в папках медиа после удаления файлов (в dry-run только показывает список).
