...
- **CHANGELOG**: всегда веди лог изменений в `CHANGELOG.md` — добавляй запись в секцию `[Unreleased]` при любых заметных изменениях (фичи, исправления, конфиг).
- **Коммиты**: после изменений кода больше чем на 5 строк — сохраняй изменения в коммит (commit message на русском).
- Перед коммитом:
